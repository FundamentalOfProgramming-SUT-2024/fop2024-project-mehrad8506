...
int move_player(PLAYER * player , char floor[50][180] , char shown_floor[50][180] , GOLD gold[6] , int * floor_level , ROOM rooms[6] , TRAP trap[3] , PASS_CHARNGER password , MONSTER * monster , DAGGER * dagger , FOOD * food);
...
int main()
{
    ...
    FOOD salam[4];
    PLAYER player1 = {-1 , -1 , 0 , 30 ,  0 , 0 , 0 , 0 , 0 , 0 , salam , 0 , PTHREAD_MUTEX_INITIALIZER};
    ...
    while(end_program < 4)
    {
        ...
        food_basics(&food);
        floor[end_program][food.y_pos][food.x_pos] = food.symbol;
        ...
        move_player(&player1 , floor[end_program] , shown_floor[end_program] , gold , &end_program , rooms[end_program] , trap , password , &monster , &dagger , &food);
//        end_program = 5;
    }
    getch();
    endwin();
    ...
    while(1)
    {
        ...
        if(player->hunger_timer > 10)
        {
            if(player->hunger > 30)
            {
                ...
                move(0 , 0);
                clrtoeol();
                mvprintw(0 , 0 , "YOU ARE HUNGRY. EAT FOOD. CURRENT HEALTH = %d" , player->health);
            }
            else
            {
                player->hunger++;
